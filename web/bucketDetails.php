...
$query = "SELECT finished,host,event from events where finished > now()-interval '1 day'*$days and bucket_id $bucket order by finished desc limit 500";
if (isset($_GET["n"]))
{
	$query = "SELECT count(*),max(finished),normalize_query(event) from events where finished > now()-interval '1 day'*$days and bucket_id $bucket group by normalize_query(event) order by count(*)  desc limit 500";
}
...
echo "(back to the <a href='overview.php?days=$days'>overview)<p>";
